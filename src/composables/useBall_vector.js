...
export function useBall() {
  ...
  const p = ref(new Vector(ww.value / 2, wh.value / 2))
  const v = ref(new Vector(50, 3))
  const a = ref(new Vector(0, 1))
  const r = ref(50)
  ...
  const update = () => {
    ...
    p.value = p.value.add(v.value)
    v.value = v.value.add(a.value)
    v.value = v.value.mul(controls.value.fade)
